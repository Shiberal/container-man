...
Receiver (`nodeB`, `192.168.1.11`) simplest:

`cm containers transfer receive`

Sender (`nodeA`, `192.168.1.10`) simplest:

`cm containers transfer send --container webapp --host 192.168.1.11`

Default values:
- port = `9000`
- sender package path = `/tmp/<container>.cm.tgz`
- receiver package path = `/tmp/cm-received-container.cm.tgz`
...
`cm containers transfer send --container webapp --package /tmp/webapp.cm.tgz --host 192.168.1.11 --port 9000 --bind-host 192.168.10.122 --wait-confirm-port 9100 --move --yes`
...
- `--move` only removes source after receiver confirms container is running and healthy (or has no healthcheck).
